...
Get-ChildItem -Path $([IO.Path]::GetFullPath([IO.Path]::Combine($((Get-Item $PSScriptRoot).Parent.FullName),'build'))) *.img | ForEach-Object { Remove-Item -Path $_.FullName }
...
################################
# Preparing floppy/disk images #
################################
Write-Host "- Preparing floppy/dism images";
$floppyimg = [IO.Path]::GetFullPath([IO.Path]::Combine($((Get-Item $PSScriptRoot).Parent.FullName),'build') + '\BOOTFL.IMG');
$floppyimgrd = [IO.Path]::GetFullPath([IO.Path]::Combine($((Get-Item $PSScriptRoot).Parent.FullName),'build') + '\BOOTFLRD.IMG');
$floppymaster = [IO.Path]::GetFullPath([IO.Path]::Combine($((Get-Item $PSScriptRoot).Parent.FullName),'tools') + '\FDOEM.144.gz');
$floppymasterrd = [IO.Path]::GetFullPath([IO.Path]::Combine($((Get-Item $PSScriptRoot).Parent.FullName),'tools') + '\FDRD.144.gz');

if(Test-path $floppyimg) { Remove-Item $floppyimg; }
if(Test-path $floppyimgrd) { Remove-Item $floppyimgrd; }

$proc_floppy = Start-Process -NoNewWindow -WorkingDirectory $(Get-Item $PSScriptRoot) 'cmd' -ArgumentList "/c $([IO.Path]::GetFullPath([IO.Path]::Combine($((Get-Item $PSScriptRoot).Parent.FullName),'tools') + '\unxutils\gunzip.exe')) < $($floppymaster) > $($floppyimg)" -PassThru;
$proc_floppy.WaitForExit();
$proc_floppy = Start-Process -NoNewWindow -WorkingDirectory $(Get-Item $PSScriptRoot) 'cmd' -ArgumentList "/c $([IO.Path]::GetFullPath([IO.Path]::Combine($((Get-Item $PSScriptRoot).Parent.FullName),'tools') + '\unxutils\gunzip.exe')) < $($floppymasterrd) > $($floppyimgrd)" -PassThru;
$proc_floppy.WaitForExit();
